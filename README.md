...
課題要件に対して必要な API を小さく実装するため、以下は採用していません。

- OpenAPI / Swagger UI: エンドポイント数が少なく、README の表と curl 例で利用方法を確認できるため。
- アプリケーション独自のエラーコード: フロントエンド実装は要件外であり、HTTP ステータスと ProblemDetail の `detail` で十分と判断したため。
- 一覧取得 / 削除 / 検索 / ページネーション: 課題で求められた機能範囲外のため。
- 認証 / 認可 / CORS: 今回の API 仕様に含まれないため。
- 構造化ログ / メトリクス / トレーシング: 本番運用を想定した監視基盤との連携は課題範囲外のため。
